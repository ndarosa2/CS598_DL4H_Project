...
class Network(nn.Module):
    def __init__(self):
        ...
        ARGS.inputdim = ARGS.numberOfInputCCSInts # NJD - added in 
...
def train():
    ...
    torch.cuda.set_device(0) # NJD - changed from torch.cuda.set_device(1) to torch.cuda.set_device(0) since only have 1 GPU
    ...
    for ind_i in range(0, k):
        ...
        with torch.cuda.device(0):# NJD - changed from torch.cuda.device(1) to torch.cuda.evice(0) since only have 1 GPU
            # Hyperparameters :
            epochs = ARGS.nEpochs
    ...
    pytorch_total_params = sum(p.numel() for p in model.parameters()) # NJD - added in 
    print("Total Number of Parameters: " + str(pytorch_total_params)) # NJD - added in 
...
def parse_arguments():
    ...
    # NJD - From original version of .py, changed default hiddenDimSize from 50 to 10000 to match value used as described in paper / what was used for diagnoses predictions
    parser.add_argument('--hiddenDimSize', type=int, default=10000, help='Size of FFN hidden layer')
    # NJD - From original version of .py, changed default batch size from 50 to 100 to match value as described in paper / what was used for diagnoses predicitons
    parser.add_argument('--batchSize', type=int, default=100, help='Batch size.')
    # NJD - From original version of .py, changed default number of epochs from 100 to 5000 to match value as described in paper / what was used for diagnoses predicitons
    parser.add_argument('--nEpochs', type=int, default=5000, help='Number of training iterations.')
    # NJD - From original version of .py, changed default learning rate from 0.001 to 0.01 to match value that was used for diagnoses predicitons. 
